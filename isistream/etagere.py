...
from backbone.shared.etagere import EtagereConfig, EtagereZone, crop_cell
...
class EtagereDetector:
    ...
    def _crop(self, frame: Frame, zone: EtagereZone, rect: tuple[float, float, float, float],
              angle_deg: float = 0.0):
        """One cell's crop — see :func:`backbone.shared.etagere.crop_cell` (shared
        with the dashboard's étagère panel so both show the same pixels)."""
        return crop_cell(frame.image, zone.frame_wh, rect, angle_deg,
                         self._cfg.model.crop_margin)
