...
```
 ...
 │  cameras → vision → metadata│  isi/zone_a/…│
 └─────────────────────────────┘            │
 ┌─────────────────────────────┐            ▼         ┌────────────────────┐      ┌──────────┐
 │ Backbone  node_id = dock_1  │── publish ─┼────────►│  MQTT broker :1883 │      │  AGVs    │
 │  cameras → vision → metadata│  isi/dock_1/…│        │  (Mosquitto)       │      │  WMS     │
 └─────────────────────────────┘            │         └─────────┬──────────┘      │  HMI     │
 ┌─────────────────────────────┐            │                   │ subscribe isi/# └────┬─────┘
 │ Backbone  node_id = cold_3  │── publish ─┘                   ▼                      │
 │  …                          │                       ┌────────────────────┐   GET   │
 └─────────────────────────────┘                       │  isi-gateway :8080 │◄────────┘
                                                        │  cache per node_id │  /nodes /tracks
        MQTT over the LAN (intranet, plaintext)         │  REST polling API  │  /zones /passings
...
Everything a node emits is namespaced under `isi/<node_id>/…`:
```
isi/zone_a/track2d/person          isi/zone_a/diagnostics/heartbeat
isi/zone_a/track3d/forklift        isi/zone_a/config           (retained)
isi/zone_a/zones/dock_door/passings
isi/zone_a/images/dock_door/42
```
...
**Method:** each node prefixes every topic with `isi/<node_id>`. The gateway reads
the `node_id` back out of the topic and tags every record with it. Global identity
is the pair **`(node_id, track_id)`**.
▶ **Expected result:** `track 42` from `zone_a` and `track 42` from `dock_1` never
collide; `/tracks` shows both, distinctly tagged.
...
(default 15 s).
▶ **Expected result:** kill a PC and within ~15 s `/nodes` flips it
...
**Method:** a single integer `schema_version`; consumers accept a set
(currently `{3, 4}`); adding optional fields is non-breaking.
...
`{"ok":true}`; `GET /nodes` → `{"nodes":[],"count":0}` (no PCs yet).

### Stage B — A PC starts publishing
Run a Backbone with `node_id: zone_a` and an mqtt sink pointing at the server.
▶ **Expected:**
- The broker shows `isi/zone_a/config` (retained), then `…/diagnostics/heartbeat`
  every ~5 s, then `…/track2d/<cls>` once detections begin.
- `GET /nodes` → `zone_a`, status **`alive`**, with its area/mode/cameras.
- `GET /config` → `zone_a`'s self-description.

### Stage C — Steady state, multiple PCs
Each PC publishes independently under its own prefix.
▶ **Expected:** `GET /nodes` lists every PC; `GET /tracks` returns every track
warehouse-wide, each tagged with its `node_id`; `GET /tracks?zone=<name>` returns
only tracks currently inside that polygon.

### Stage D — An AGV consumes
The AGV polls `GET /tracks` (or `?zone=`) on its own loop.
...
▶ **Expected:** within ~15 s that node flips to **`stale`** in `/nodes`; its last
...
▶ **Expected:** it appears in `/nodes`/`/zones` on its own; **nothing on the
server changes.**
...
| `isi_gateway/tests` | 47 | subscriber cache, every REST route, liveness, **import discipline** |
| Backbone full suite | **488** | no regression across the pipeline |
| monitor_web (dashboard consumer) | **272** | the operator UI consumes the contract |

▶ **Result:** the *logic* of every component is correct in isolation.
...
```
node zone_a ─┐                         ┌─ GET /nodes  → zone_a + zone_b, both alive
node zone_b ─┼─► mosquitto :1883 ─► gateway ─┤ GET /config → both retained adverts
...
broker and appeared in the *containerised* gateway's `/nodes`. **This run caught a
...
3. **API surface** — are the current endpoints (`/nodes /tracks /zones /passings
   /diagnostics /config`) sufficient for the WMS/AGV integration, or is a
   push/streaming (WebSocket) variant wanted alongside polling?
