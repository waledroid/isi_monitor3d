...
class ProjectConfig(BaseModel):
    ...
    # How Phase 6/7 synthesize: "auto" picks copy_paste+sdxl_inpaint when the project
    # has background images (real cutouts on real scenes — robust when depth can't see
    # the object), else the depth path (depth_remix + sdxl_controlnet). "copy_paste" /
    # "depth" force the pair. See resolve_synthesis().
...
def create_project(data_dir: Path, name: str, classes: list[ClassSpec], *,
                   mode: Literal["generate", "label"] = "generate",
                   synthesis_mode: Literal["auto", "copy_paste", "depth"] = "auto") -> Path:
    ...
    generation) — the export defaults to the labelme exporter. ``synthesis_mode``
    chooses the Phase 6/7 path (``auto`` = copy_paste+inpaint when backgrounds are
    ingested, else the depth path). Refuses to overwrite an existing project."""
    ...
    cfg = ProjectConfig.model_validate(
        {**template, "name": name, "mode": mode, "synthesis_mode": synthesis_mode,
...
# copy_paste scaffolds pair with the sdxl_inpaint generator; depth_remix/box3d_procedural
# pair with sdxl_controlnet. The pair must stay matched (copy_paste emits a base+inpaint
# region that only the inpaint generator consumes; depth sources emit a depth control map
# the controlnet generator follows), so a single setting drives both.
_PASTE_PAIR = (["copy_paste"], "sdxl_inpaint")
_DEPTH_GENERATOR = "sdxl_controlnet"


def resolve_synthesis(project: ProjectConfig, project_dir: Path) -> tuple[list[str], str, dict]:
    """Resolve the (scaffold sources, generator) pair from ``synthesis_mode`` + bg presence.

    - ``copy_paste`` or (``auto`` and the project has background images) →
      ``(["copy_paste"], "sdxl_inpaint")`` — paste real cutouts onto real scenes and
      harmonize the seam (robust when monocular depth can't represent the object);
    - otherwise → the configured depth sources (``copy_paste`` filtered out, default
      ``["depth_remix"]``) + ``"sdxl_controlnet"``.

    Returns ``(sources, generator, info)`` where ``info`` (``mode``/``bg_count``/
    ``path``) is for the Studio badge. Counts backgrounds from the manifest (lazy
    import to avoid a cycle).
    """
    from .manifest import Manifest

    mode = getattr(project, "synthesis_mode", "auto")
    bg_count = sum(1 for r in Manifest.load(project_dir).active() if r.background)
    use_paste = mode == "copy_paste" or (mode == "auto" and bg_count > 0)
    if use_paste:
        sources, generator = list(_PASTE_PAIR[0]), _PASTE_PAIR[1]
    else:
        configured = project.phase("scaffolds").get("sources", []) or []
        sources = [s for s in configured if s != "copy_paste"] or ["depth_remix"]
        generator = _DEPTH_GENERATOR
    info = {"mode": mode, "bg_count": bg_count, "sources": sources, "generator": generator,
            "path": "copy_paste + sdxl_inpaint" if use_paste else "depth + sdxl_controlnet"}
    return sources, generator, info
