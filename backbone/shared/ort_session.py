...
# TensorRT is used ONLY through native `.engine` files (suffix dispatch below;
# build them with tools/onnx_to_engine.py). The former lazy TRT-EP path —
# ISI3D_TRT / detection.trt_enabled silently compiling engines behind `.onnx`
# paths, minutes of dead air per model/shape — was retired 2026-07-23: a
# `.onnx` now ALWAYS means plain ONNX Runtime (CUDA/CPU), predictably.
...
def resolve_providers(providers):
    ...
    Each ``CUDAExecutionProvider`` entry becomes a ``(name, options)`` tuple;
    every other provider passes through unchanged. No TensorRT EP is ever
    injected — TensorRT runs only via native ``.engine`` files (suffix
    dispatch in :func:`build_onnx_session`).
    """
    return [(p, dict(_CUDA_OPTIONS)) if p == "CUDAExecutionProvider" else p
            for p in providers]
