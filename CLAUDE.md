...
python -m backbone.runtime --config config/backbone.yaml   # run the Backbone (CLI entry; SIGINT/SIGTERM to stop)
...
**RTSP ingest backend (S2, implemented).** S2's RTSP path uses GStreamer driven from Python via **PyGObject** (`import gi; gi.require_version("Gst","1.0"); from gi.repository import Gst, GLib`), not `gst-python` and not `cv2.CAP_GSTREAMER` (the conda-forge OpenCV build is FFmpeg-only — no GStreamer backend compiled in). We avoided `gst-python` because its only py3.10 conda-forge build is 1.21.x, which transitively pins FFmpeg 5 and conflicts with OpenCV 4.13's FFmpeg 8 dep. PyGObject is gstreamer-version-agnostic and binds to whatever GStreamer's GIR files describe at runtime. The Hikvision low-latency pipeline (in `backbone/ingestion/rtsp.py:PIPELINE_TEMPLATE`): `rtspsrc latency=100 drop-on-latency=true protocols=tcp ntp-sync=true ! rtph264depay ! avdec_h264 ! videoconvert ! video/x-raw,format=BGR ! appsink emit-signals=true sync=false max-buffers=1 drop=true`. Each `RtspFrameSource` owns one `Gst.Pipeline` + `GLib.MainLoop` in a daemon thread; the appsink callback decodes BGR and pushes into an internal `Queue(maxsize=1, drop-old)`.
...
A **separate FastAPI process** (sibling project under `monitor_web/`) — the operator UI. Consumes the Backbone over the UDP/JSON bus + shared YAML; per the process-boundary rule it imports only consumer-side helpers (`backbone.metadata.schemas`, `backbone.shared.zones`, `backbone.ingestion`), **never** `backbone.runtime/homography/triangulation`, and imports `backbone.detection` only in **one documented place** — `monitor_web/detection_overlay.py`, reused by the live CAM `?detect=1` preview and the hidden MP4 viewer (both below).
...
- **Stack:** FastAPI + Jinja2 + HTMX + Material Web (CDN) + **Pixi.js** for the floor map + **Alpine.js** (CDN) driving the big-panel reactive state (`static/js/big_panel.js` = an `Alpine.store('bigPanel')`; markup binds via `x-show`/`:class`/`:src`/`@click`). `big_panel.js` is loaded as a **classic deferred script immediately before the Alpine CDN script** so its `alpine:init` listener registers first (module-vs-defer order is otherwise ambiguous → store never registers → bindings error → views hidden). Session persistence (view + mp4 pick) is plain `sessionStorage` via `Alpine.effect` (no persist plugin). No Node toolchain.
- **Layout:** big panel with a **MAP / CAM 1 / CAM 2** segmented toggle (Pixi 2D digital-twin map + per-camera MJPEG via `/stream/video/{cam}`), LOGS + STATUS sidebar, START/STOP (spawns/kills the Backbone subprocess), GB/FR i18n.
...
- **Endpoints:** `/api/status`, `/api/config` (GET/POST cameras+detection+zones), `/api/zones`, `/api/danger-zones-object`, `/api/cameras/available`, `/api/ui-settings`, `/api/logs`, `/api/control/{start,stop}`, `/stream/video/{cam}` (MJPEG; **`?detect=1`** overlays detection boxes in-process — **no calibration / Backbone START needed** — via `detection_overlay.py`), `/stream/mp4/{name}` (annotated, hidden dev viewer), `/ws/tracks`.
- **Live detect preview (S13.2):** CAM tabs stream `/stream/video/{cam}?detect=1` — the dashboard runs the detector (`detection_overlay.get_detector`) per frame and draws boxes; **no calibration** (unlike the full metric Backbone, which requires `calibration_path` and emits metric `Track2D`, not pixel boxes). If no model is configured it **auto-falls-back to the latest trained `best.onnx`** (`trainer/isidet/runs/.../weights/best.onnx`) with class `palette_vide`; falls back to the raw feed only if no model exists at all. Model changes in Settings call `reset_detector()` so the preview updates without a restart.
