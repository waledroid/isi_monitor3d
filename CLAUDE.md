...
- **Cam views are POSE-ONLY:** the big CAM 1/CAM 2 views never run a full-frame object detector (the old "no zones → full detection" fallback was removed). Once the Backbone is RUNNING, the only model on the full frame is the **human-pose** ONNX (`detection.pose_onnx_path`); object boxes come **solely from the zone workers' snapshots** (already remapped to source-frame pixels), plus distance lines/occupancy derived from them — no zones ⇒ skeletons only. `detection_overlay.get_detector` (full-frame, with its latest-`best.onnx` fallback) is now used only by the MP4 dev viewer and the warp-verification path.
