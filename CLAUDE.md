...
interface. isicomms serves a live probe UI at `:8080/ui` (tables incl. a
Consumers card + schema tree + AGV test cards; `/test` redirects here) and
Swagger at `/docs`.
...
python -m isistream --config config/backbone.yaml          # isistream — the perception producer (capture+decode+detect+pose → detection sets)
...
- `monitor3d` conda env — everything the Backbone runtime and tests need. Activated for every dev / test / lint command above. Includes Python 3.10, OpenCV 4.13 (headless py310), GStreamer 1.28 + plugins, PyGObject, FilterPy, scipy, numpy, PyYAML, pytest/ruff, **CUDA 12.9 + ONNX Runtime 1.23.2 (pip onnxruntime-gpu wheel; TensorRT 10.16 via pip tensorrt-cu12) on the RTX 5070 Blackwell sm_120. Model backend is SUFFIX-DISPATCHED (`build_onnx_session`): a `.onnx` path runs plain ORT CUDA fp16, a `.engine` path runs a prebuilt native TensorRT engine (`backbone/shared/trt_session.py`; build with `tools/onnx_to_engine.py`, ~2.1-2.3x over CUDA EP, loads in ~1-2 s, per-machine artifact with a JSON sidecar). The lazy TRT-EP (`detection.trt_enabled` / ISI3D_TRT / models/.trt_cache) was retired 2026-07-23 — it silently compiled engines for minutes on every new model/shape**, **OpenVINO** (S13 `yolo_openvino` detector — CPU/Intel-iGPU only, imported lazily), **pydantic 2.x** (S6 metadata schemas).
...
- The producer runs as **its own process, never in-process**: measured on the rig, the same tick costs ~55 ms standalone vs ~2,200 ms inside the dashboard (GIL + ORT-pool contention). `isistream/` is FastAPI-free by construction; monitor_web only supervises it (config-save hot-restarts it so Settings model changes apply live).
- **Shared frame bus (decode once, fan out):** the producer publishes every decoded frame to `/dev/shm/isi3d_frame_<cam>` (`backbone/shared/frame_shm.py` — double-buffered seqlock, lock-free readers); the dashboard's camera hub PREFERS the bus (zero RTSP session of its own while the producer runs, re-checks every 5 s) and falls back to its own source when the bus is absent/stale (backbone stopped, frames mode, pre-START preview). ONE RTSP session + ONE decode per camera system-wide; panels show the exact pixels the models saw. (A future consumer can read the bus directly via `FrameShmReader`; the speculative `shm` FrameSource plugin was removed as dead code — resurrect from git history if a registry-visible source is ever needed.)
...
**Latency note.** Solo emission is LATEST-FRAME-ONLY via a sticky per-camera degraded flag: Mode 1 (single camera configured) emits every frame immediately (no `degraded_emit_after_ms` tax); Mode 2 degradation pays the wait once at entry — the NEWEST buffered frame emits, older ones are discarded — then streams the survivor at full input fps with zero buffering. The flag clears automatically when aligned pairing resumes; if ALL cameras are degraded (startup stall), a probe un-degrades one camera every 10× `degraded_emit_after_ms` so alignment can re-form (fixed 2026-08-06 — before that the all-degraded state was a permanent-solo deadlock that starved cross-cam fusion and Track3D).
